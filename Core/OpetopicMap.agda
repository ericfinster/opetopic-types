...
module Core.OpetopicMap where
  ...
  Cns⇒ σ {●} c = tt*
  Cns⇒ (σₙ , σₛₙ) {𝑜 ∣ ._} (lf x) = lf (σₛₙ x)
  Cns⇒ (σₙ , σₛₙ) {𝑜 ∣ ._} (nd x c y d z ψ) = 
  ...
  [_⊙_]c σ {●} c↓ = tt*
  [_⊙_]c (σₙ , σₛₙ) {𝑜 ∣ 𝑝} {c = lf x} idp = idp
  [_⊙_]c (σₙ , σₛₙ) {𝑜 ∣ 𝑝} {c = nd x c y d z ψ} (c↓ , y↓ , d↓ , z↓ , ψ↓ , idp) =
