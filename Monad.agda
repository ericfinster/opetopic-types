...
module Monad where
  ...
  postulate
    ...
    η-pos-elim : (M : 𝕄) (f : Idx M)
      → (X : (p : Pos M (η M f)) → Set)
      → (η-pos* : X (η-pos M f))
      → (p : Pos M (η M f)) → X p

    μ : (M : 𝕄) {f : Idx M} (σ : Cns M f)
      → (δ : (p : Pos M σ) → Cns M (Typ M σ p))
      → Cns M f

    μ-pos : (M : 𝕄) {f : Idx M} (σ : Cns M f)
      → (δ : (p : Pos M σ) → Cns M (Typ M σ p))
      → (p : Pos M σ) (q : Pos M (δ p))
      → Pos M (μ M σ δ)

    μ-pos-fst : (M : 𝕄) {f : Idx M} (σ : Cns M f)
      → (δ : (p : Pos M σ) → Cns M (Typ M σ p))
      → Pos M (μ M σ δ) → Pos M σ

    μ-pos-snd : (M : 𝕄) {f : Idx M} (σ : Cns M f)
      → (δ : (p : Pos M σ) → Cns M (Typ M σ p))
      → (p : Pos M (μ M σ δ))
    ...
    μ-pos-assoc : (M : 𝕄) {f : Idx M} (σ : Cns M f)
      → (δ : (p : Pos M σ) → Cns M (Typ M σ p))
      → (ε : (p : Pos M (μ M σ δ)) → Cns M (Typ M (μ M σ δ) p))
    ...
    μ-pos-fst-unit-right : (M : 𝕄) {f : Idx M}
      → (σ : Cns M f) (p : Pos M σ)
    ...
    μ-pos-fst-unit-left : (M : 𝕄) (f : Idx M) 
      → (δ : (p : Pos M (η M f)) → Cns M f)
      → (p : Pos M (δ (η-pos M f)))
      → μ-pos-fst M (η M f) δ p ↦ η-pos M f
    {-# REWRITE μ-pos-fst-unit-left #-}

    -- μ-pos-fst-assoc : (M : 𝕄) {f : Idx M} (σ : Cns M f)
    --   → (δ : (p : Pos M σ) → Cns M (Typ M σ p))
    --   → (ε : (p : Pos M (μ M σ δ)) → Cns M (Typ M (μ M σ δ) p))
  ...
  γ : (M : 𝕄) {f : Idx M} {σ : Cns M f}
    → (ρ : Cnsₛ M (f , σ))
    → (δ : (p : Pos M σ) → Cns M (Typ M σ p))
    → (ε : (p : Pos M σ) → Cnsₛ M (Typ M σ p , δ p))
    → Cnsₛ M (f , μ M σ δ)

  γ-pos-inl : (M : 𝕄) {f : Idx M} {σ : Cns M f} 
    → (ρ : Cnsₛ M (f , σ))
    → (δ : (p : Pos M σ) → Cns M (Typ M σ p))
    → (ε : (p : Pos M σ) → Cnsₛ M (Typ M σ p , δ p))
    → Posₛ M ρ → Posₛ M (γ M ρ δ ε)
  
  γ-pos-inr : (M : 𝕄) {f : Idx M} {σ : Cns M f} 
    → (ρ : Cnsₛ M (f , σ))
    → (δ : (p : Pos M σ) → Cns M (Typ M σ p))
    → (ε : (p : Pos M σ) → Cnsₛ M (Typ M σ p , δ p))
  ...
  γ-pos-elim : (M : 𝕄) {f : Idx M} {σ : Cns M f} 
    → (ρ : Cnsₛ M (f , σ))
    → (δ : (p : Pos M σ) → Cns M (Typ M σ p))
    → (ε : (p : Pos M σ) → Cnsₛ M (Typ M σ p , δ p))
  ...
  postulate
    ...
    -- γ elim comp rules
    γ-pos-elim-inl-β : (M : 𝕄) {f : Idx M} {σ : Cns M f} 
      → (ρ : Cnsₛ M (f , σ))
      → (δ : (p : Pos M σ) → Cns M (Typ M σ p))
      → (ε : (p : Pos M σ) → Cnsₛ M (Typ M σ p , δ p))
      → (X : Posₛ M (γ M ρ δ ε) → Set)
    ...
    γ-pos-elim-inr-β : (M : 𝕄) {f : Idx M} {σ : Cns M f} 
      → (ρ : Cnsₛ M (f , σ))
      → (δ : (p : Pos M σ) → Cns M (Typ M σ p))
      → (ε : (p : Pos M σ) → Cnsₛ M (Typ M σ p , δ p))
      → (X : Posₛ M (γ M ρ δ ε) → Set)
  ...
  η-pos-elimₛ : (M : 𝕄) (f : Idxₛ M)
    → (X : (p : Posₛ M (ηₛ M f)) → Set)
    → (η-pos* : X (η-posₛ M f))
    → (p : Posₛ M (ηₛ M f)) → X p
  η-pos-elimₛ M f X η-pos* (inl unit) = η-pos*

  μₛ : (M : 𝕄) {f : Idxₛ M} (σ : Cnsₛ M f)
    → (δ : (p : Posₛ M σ) → Cnsₛ M (Typₛ M σ p))
    → Cnsₛ M f
  μₛ M (lf τ) κ = lf τ
  μₛ M (nd σ δ ε) κ =
    let w = κ (inl unit)
  ...
  μ-posₛ : (M : 𝕄) {f : Idxₛ M} (σ : Cnsₛ M f)
    → (δ : (p : Posₛ M σ) → Cnsₛ M (Typₛ M σ p))
  ...
  μ-pos-fstₛ : (M : 𝕄) {f : Idxₛ M} (σ : Cnsₛ M f)
    → (δ : (p : Posₛ M σ) → Cnsₛ M (Typₛ M σ p))
  ...
  μ-pos-sndₛ : (M : 𝕄) {f : Idxₛ M} (σ : Cnsₛ M f)
    → (δ : (p : Posₛ M σ) → Cnsₛ M (Typₛ M σ p))
    → (p : Posₛ M (μₛ M σ δ))
  ...
  postulate
    ...
    Typ-Slice : (M : 𝕄) 
      → {f : Idx (Slice M)}
      → (σ : Cns (Slice M) f) (p : Pos (Slice M) σ)
