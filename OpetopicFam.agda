--
--  OpetopicFam.agda - Dependent families of opetopic types
--
...
module OpetopicFam where

  𝕆Fam : ∀ {n ℓ₀} (Γ : 𝕆Type n ℓ₀)
    → (ℓ : Level) → Type (ℓ-max ℓ₀ (ℓ-suc ℓ))

  Frm↓ : ∀ {n ℓ₀ ℓ} {Γ : 𝕆Type n ℓ₀} (X : 𝕆Fam Γ ℓ)
    → {𝑜 : 𝒪 n} (f : Frm Γ 𝑜) → Type ℓ
    
  Cns↓ : ∀ {n ℓ₀ ℓ} {Γ : 𝕆Type n ℓ₀} (X : 𝕆Fam Γ ℓ)
    → {𝑜 : 𝒪 n} {f : Frm Γ 𝑜} (f↓ : Frm↓ X f)
    → {𝑝 : 𝒫 𝑜} (c : Cns Γ f 𝑝) → Type ℓ 

  Shp↓ : ∀ {n ℓ₀ ℓ} {Γ : 𝕆Type n ℓ₀} (X : 𝕆Fam Γ ℓ)
    → {𝑜 : 𝒪 n} {f : Frm Γ 𝑜} {f↓ : Frm↓ X f}
    → {𝑝 : 𝒫 𝑜} {c : Cns Γ f 𝑝} (c↓ : Cns↓ X f↓ c)
    → (p : Pos 𝑝) → Frm↓ X (Shp Γ c p) 

  η↓ : ∀ {n ℓ₀ ℓ} {Γ : 𝕆Type n ℓ₀} (X : 𝕆Fam Γ ℓ)
    → {𝑜 : 𝒪 n} {f : Frm Γ 𝑜} (f↓ : Frm↓ X f)
    → Cns↓ X f↓ (η Γ f)

  {-# TERMINATING #-}
  μ↓ : ∀ {n ℓ₀ ℓ} {Γ : 𝕆Type n ℓ₀} (X : 𝕆Fam Γ ℓ)
    → {𝑜 : 𝒪 n} {f : Frm Γ 𝑜} {f↓ : Frm↓ X f}
    → {𝑝 : 𝒫 𝑜} {c : Cns Γ f 𝑝} (c↓ : Cns↓ X f↓ c)
    → {𝑞 : (p : Pos 𝑝) → 𝒫 (Typ 𝑝 p)}
  ...
  postulate
    ...
    η↓-shp : ∀ {n ℓ₀ ℓ} {Γ : 𝕆Type n ℓ₀} (X : 𝕆Fam Γ ℓ)
      → {𝑜 : 𝒪 n} {f : Frm Γ 𝑜} (f↓ : Frm↓ X f)
      → (p : Pos (ηₒ 𝑜))
      → Shp↓ X (η↓ X f↓) p ↦ f↓
    {-# REWRITE η↓-shp #-}

    μ↓-shp : ∀ {n ℓ₀ ℓ} {Γ : 𝕆Type n ℓ₀} (X : 𝕆Fam Γ ℓ)
      → {𝑜 : 𝒪 n} {f : Frm Γ 𝑜} (f↓ : Frm↓ X f)
      → {𝑝 : 𝒫 𝑜} {c : Cns Γ f 𝑝} (c↓ : Cns↓ X f↓ c)
      → {𝑞 : (p : Pos 𝑝) → 𝒫 (Typ 𝑝 p)}
      → {d : (p : Pos 𝑝) → Cns Γ (Shp Γ c p) (𝑞 p)}
    ...
    μ↓-unit-r : ∀ {n ℓ₀ ℓ} {Γ : 𝕆Type n ℓ₀} (X : 𝕆Fam Γ ℓ)
      → {𝑜 : 𝒪 n} {f : Frm Γ 𝑜} {f↓ : Frm↓ X f}
      → {𝑝 : 𝒫 𝑜} {c : Cns Γ f 𝑝} (c↓ : Cns↓ X f↓ c)
      → μ↓ X c↓ (λ p → η↓ X (Shp↓ X c↓ p)) ↦ c↓
    {-# REWRITE μ↓-unit-r #-} 

    μ↓-unit-l : ∀ {n ℓ₀ ℓ} {Γ : 𝕆Type n ℓ₀} (X : 𝕆Fam Γ ℓ)
      → {𝑜 : 𝒪 n} {f : Frm Γ 𝑜} {f↓ : Frm↓ X f}
    ...
    μ↓-assoc : ∀ {n ℓ₀ ℓ} {Γ : 𝕆Type n ℓ₀} (X : 𝕆Fam Γ ℓ)
      → {𝑜 : 𝒪 n} {f : Frm Γ 𝑜} {f↓ : Frm↓ X f}
      → {𝑝 : 𝒫 𝑜} {c : Cns Γ f 𝑝} (c↓ : Cns↓ X f↓ c)
      → {𝑞 : (p : Pos 𝑝) → 𝒫 (Typ 𝑝 p)}
      → {d : (p : Pos 𝑝) → Cns Γ (Shp Γ c p) (𝑞 p)}
  ...
  module _ {n ℓ₀ ℓ} {Γₙ : 𝕆Type n ℓ₀} {Γₛₙ : {𝑜 : 𝒪 n} (f : Frm Γₙ 𝑜) → Type ℓ₀}
           (Xₙ : 𝕆Fam Γₙ ℓ) (Xₛₙ : {𝑜 : 𝒪 n} {f : Frm Γₙ 𝑜} (f↓ : Frm↓ Xₙ f) → Γₛₙ f → Type ℓ)
    where
  ...
  𝕆Fam {n = zero} Γ ℓ = Lift Unit
  𝕆Fam {n = suc n} (Γₙ , Γₛₙ) ℓ =
    Σ[ Xₙ ∈ 𝕆Fam Γₙ ℓ ]
  ...
  record 𝕆Fam∞ {n ℓ₀ ℓ₁} {Γₙ : 𝕆Type n ℓ₀} (Γ : 𝕆Type∞ ℓ₀ Γₙ)
      (Xₙ : 𝕆Fam Γₙ ℓ₁) : Type (ℓ-max ℓ₀ (ℓ-suc ℓ₁)) where
    coinductive
    field
      FillTy : {𝑜 : 𝒪 n} {f : Frm Γₙ 𝑜} → Frm↓ Xₙ f → Fill Γ f → Type ℓ₁
      HomTy : 𝕆Fam∞ (Hom Γ) (Xₙ , FillTy)
