...
module Experimental.cwf.Opetopes where
  ...
  𝒪 : ℕ → Type
  𝒫 : {n : ℕ} (X : 𝒪 n → Type) → 𝒪 n → Type
  Pos : {n : ℕ} (X : 𝒪 n → Type) {𝑜 : 𝒪 n} → 𝒫 X 𝑜 → Type 
  Typ : {n : ℕ} (X : 𝒪 n → Type) {𝑜 : 𝒪 n} (𝑝 : 𝒫 X 𝑜) (p : Pos X 𝑝) → 𝒪 n

  𝒪 zero = Unit
  𝒪 (suc n) = Σ[ 𝑜 ∈ 𝒪 n ] 𝒫 (const Unit) 𝑜
  ...
  data Pd {n : ℕ} (X : 𝒪 (suc n) → Type) : 𝒪 (suc n) → Type where

    lfₒ : {𝑜 : 𝒪 n} → Pd X (𝑜 , ηₒ (const Unit) tt)
      
    ndₒ : {𝑜 : 𝒪 n} (𝑝 : 𝒫 (𝒫 (const Unit)) 𝑜)
      → (pd : 𝒫 (λ o → Σ[ p ∈ 𝒫 (const Unit) o ] Pd X (o , p)) 𝑜)
      -- → X (𝑜 , ν₀ (𝒫 (const Unit)) (const Unit) (λ _ _ → tt) 𝑜 𝑝) 
      → Pd X (𝑜 , μₒ (const Unit) {𝑜 = 𝑜} 𝑝)
