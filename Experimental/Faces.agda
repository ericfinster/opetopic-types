...
module Experimental.Faces where

  data InnerFace : {n : ℕ} → 𝒪 n → ℕ → Type where
    as-src : {n : ℕ} (o : 𝒪 n) (p : 𝒫 o) (q : 𝒫 (o ∣ p))
      → Pos q → InnerFace (o ∣ p ∣ q) (suc n)
    as-tgt : {n : ℕ} (o : 𝒪 n) (p : 𝒫 o) (q : 𝒫 (o ∣ p))
      → Pos q → InnerFace (o ∣  p ∣  q) n
    raise-face : {n m : ℕ} (o : 𝒪 n) (p : 𝒫 o)
      → InnerFace o m → InnerFace (o ∣ p) m 

  data Face : {n : ℕ} → 𝒪 n → ℕ → Type where
    top : {n : ℕ} (o : 𝒪 n) → Face o n
    tgt-face : {n : ℕ} (o : 𝒪 (suc n)) → Face o n
    init : {n : ℕ} (o : 𝒪 (suc n)) → Face o 0
