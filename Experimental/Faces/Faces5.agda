...
module Lib.Faces5 where
  ...
  StemTgtCell : ∀ {n ℓ} (X : 𝕆Type n ℓ) (P : Frm X → Type ℓ)
    → (U : Frm (X , P) → Type ℓ) 
    → (f : Frm X) (s : Src P f) (t : P f)
    → (ρ : Pd U (f , s , t))
    → StemCell X P U f s t ρ (Frm⇒ (TgtIncl X P f s) (BdryFrm X f))
  StemTgtCell X P U f ._ ._ (lf tgt) = lf-cell f tgt
  StemTgtCell X P U f ._ ._ (nd src tgt flr brs) = nd-cell-here src tgt flr brs

  StemTgtSrc : ∀ {n ℓ} (X : 𝕆Type n ℓ) (P : Frm X → Type ℓ)
    → (U : Frm (X , P) → Type ℓ) 
    → (f : Frm X) (s : Src P f) (t : P f)
    → (ρ : Pd U (f , s , t)) (p : Pos P s)
    → StemCell X P U f s t ρ (Frm⇒ (SrcIncl X P f s p) (BdryFrm X (Typ P s p)))
  StemTgtSrc X P U f s t ρ p = {!!} 
  ...
  cell-map X P U f s t ρ ._ (src-cell p) = StemTgtSrc X P U f s t ρ p
  cell-map X P U f s t ρ ._ tgt-cell = StemTgtCell X P U f s t ρ
