...
module Lib.Faces10 where
  ...
  data BdryCell {n} {ℓ} X P f s where
    ...
    src-cell : (p : Pos P s) → BdryCell X P f s (StemSrcFrm X P f s p)

    tgt-cell : BdryCell X P f s (StemTgtFrm X P f s)
  ...
  -- And at this point I don't see any way to do anything without bringing
  -- back the boundary so that we have a tree which sits in the correct
  -- frame and to which we can bind the recursive call.

  -- And then there's one more last difficulty with this: we need some
  -- kind of decoding for this tree which allows us to recurs in the
  -- right way.
