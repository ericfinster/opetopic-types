...
module Native.Groupoid where
  ...
  --
  --  We need to find a case which is contractible ...
  --

  -- something : ∀ {ℓ n} (X : Type ℓ) (x : X)
  --   → (ο : 𝕆 n) (ρ : ℙ ο)
  --   → Σ[ c ∈ GrpCell X (ο , canon-frm X x ο) ]
  --     GrpCell {n = suc n} X ((ο , ρ) , (canon-frm X x ο , canon-web X x ρ , (λ p → reflₒ x (Typ ρ p)) , reflₒ x ο))
  -- something X x ο ρ = reflₒ x ο , reflₒ x (ο , ρ) 

  -- matching : ∀ {ℓ n} (X : Type ℓ) 
  --   → (ο : 𝕆 n) (ρ : ℙ ο) (τ : Tr (ο , ρ))
  --   → (f : Frm (Grp X n) ο) (s : Web (Grp X n) f ρ)
  --   → (δ : (p : Pos ρ) → GrpCell X (Typ ρ p , Shp (Grp X n) s p))
  --   → (p : Σ[ c ∈ GrpCell X (ο , f) ]
  --          GrpCell {n = suc n} X ((ο , ρ) , (f , s , δ , c)))
  --   → p ≡ {!!} 
  -- matching  = {!!}
  ...
  -- I think the next step would be to show that the same is true for Src's
  -- So I'm not sure which is going to be more convenient here.  Another option
  -- is the "root cell" which will occur when you match on s.  Either it is a
  -- leaf, in which case you get the cell it was a leaf on, or else there is a
  -- node, and you can take the cell decorating  that element.
  target : ∀ {ℓ n} (X : Type ℓ)
    → {i : Idx (Grp X n)}
  ...
  -- Okay! So I think this is most of the work.  This shows we are
  -- over the right index.  And then the next step is to complete the
  -- characterization of ω and ϵ.
  fib : ∀ {ℓ n} (X : Type ℓ)
    → {ο : 𝕆 n} (ρ : ℙ ο) (τ : Tr (ο , ρ))
